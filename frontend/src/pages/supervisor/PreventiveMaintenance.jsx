          ...
          {days.map((day, i) => {
            ...
            return (
              <div key={day} className={clsx("aspect-square rounded-[8px] min-h-[52px] flex flex-col items-center justify-start p-[6px] px-[4px]", hasTasks && "hover:bg-[rgba(59,114,246,0.08)] cursor-pointer")}>
                <div className={clsx("text-[1rem] font-bold flex items-center justify-center", isToday ? "bg-[#3B72F6] text-white w-[28px] h-[28px] rounded-full" : hasTasks ? "text-[var(--text-primary)]" : "text-[var(--text-muted)]")}>
                  {day}
                </div>
                {hasTasks && (
                  <div className="flex gap-[4px] flex-wrap justify-center mt-[6px]">
                    {dayTasks.slice(0, 4).map((t, idx) => {
                      let dotColor = '#3B82F6' // Stronger blue
                      if (t.status === 'OVERDUE') dotColor = '#EF4444' // Stronger red
                      else if (t.status === 'COMPLETED') dotColor = '#22C55E' // Stronger green
                      else if (t.type === 'CALIBRATION') dotColor = '#A855F7' // Stronger purple

                      return <div key={idx} className="w-5 h-5 rounded-full shadow-[0_1px_2px_rgba(0,0,0,0.15)] ring-1 ring-[var(--bg-card)]" style={{ backgroundColor: dotColor }} title={`${t.type} - ${t.status}`}></div>
                    })}
                  </div>
                )}
              </div>
